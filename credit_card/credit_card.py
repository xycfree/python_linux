#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Date    : 2017/2/17 15:58
# @Descript: ATM 信用卡  http://www.cnblogs.com/wushank/p/5248916.html
...
def doshopping(userobj):
    ...
    while not exitflag:
        ...
        shop_cassify_id = raw_input("请选择商品分类编号[1-3]: ").strip().lower()
        ...
        if int(shop_cassify_id) not in range(1, 6):
            ...
            while choose_goods_flag:
                ...
                goods_id = raw_input("选择商品编号,加入购物车(q返回上一级): ").strip().lower()
                ...
                if goods_id == "q":
                    ...
                    if result:
                        ...
                        nextflag = False
                        while not nextflag:
                            donext = raw_input("继续购物(y) or 返回上一级(q):").strip().lower()
                            if donext == "y":
                                break
                            elif donext == "q":
                                choose_goods_flag = False
...
def card_center(userobj):
    if userobj.islogin:
        ...
        while not input_flag:
            cardno = unicode(raw_input("请输入信用卡卡号: ").strip().lower())
            if cardno.isnumeric():
                card = CreditCard(cardno)
    ...
    while not quitflag:
        ...
        if _choose == "2":
            if card.frozenstatus == 1:
                ...
                while not quitflag:
                    ...
                    cost = unicode(cost)
                    if cost.isnumeric():  # Python isnumeric() 方法检测字符串是否只由数字组成。这种方法是只针对unicode对象
        ...
        if _choose == "3":
            if card.frozenstatus == 1:
                ...
                quitflag = False
                while not quitflag:
                    trans_cardno = unicode(common.input_msg("请输入要转账的卡号(q返回):"))
                    if trans_cardno.isnumeric():
                        ...
                        if not trans_cardobj.card_is_exists:
                            ...
                            # 卡号存在
                            trans_cost = unicode(common.input_msg("请输入要转账的金额: "))
