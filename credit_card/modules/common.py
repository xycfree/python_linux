...
def verification_code():
    ...
    _code = list()
    for i in range(4):
        ra = random.randrange(10)  # random.randrange(1,10,4) 获取一个从1到10步长为4的随机数
        if i == ra/2:
            _code.append(chr(random.randrange(97, 122)))  # 转换为英文
        else:
            _code.append(str(ra))
    result = ''.join(_code)
    return result
...
def input_msg(message, limit_value=tuple()):
    ...
    is_null_flag = True
    while is_null_flag:
        input_value = raw_input(message).strip()  # 96e79218965eb72c92a549dd5a330112
        if not input_value:
...
if __name__ == '__main__':
    ...
    # print encrypt('wang')
    print(get_chinese_num(u'wang王王wag'))