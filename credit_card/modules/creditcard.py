...
class CreditCard(object):
    __database = "{}.db".format(os.path.join(settings.DATABASE['dbpath'], settings.DATABASE['tables']['creditcard']))
    ...
    def card_pay(self, cost, paytype, sereialno):
        ...
        _tmp_bill_record = dict(cardno="{0}".format(self.cardno),
                                starttime=date.strftime(datetime.now(), "%Y-%m-%d %H:%M"),
                                payfor=payfor,
                                cost=cost,
                                serialno=sereialno)
